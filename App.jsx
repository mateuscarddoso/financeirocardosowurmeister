...
const App = () => {
  ...
  const [sortStatus, setSortStatus] = useState('none'); // 'none', 'pending', 'paid'
          ...
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              ...
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 shadow-sm">
                ...
                <p className="text-2xl font-bold text-slate-700 mt-2">{formatCurrency(stats.totalOut - stats.realOut)}</p>
              </div>
            </div>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="text-xs font-bold text-slate-400 uppercase tracking-wider bg-slate-50/50">
                      ...
                      <th className="px-2 py-3 border-b cursor-pointer hover:bg-slate-100 transition-colors select-none" onClick={() => setSortStatus(prev => prev === 'none' ? 'pending' : prev === 'pending' ? 'paid' : 'none')}>Status <span className="text-slate-400 ml-1">{sortStatus === 'pending' ? '↓' : sortStatus === 'paid' ? '↑' : '↕'}</span></th>
                  ...
                  <tbody className="divide-y divide-slate-50 font-medium">
                    {currentEntries.length === 0 ? (
                      ...
                      [...currentEntries].sort((a, b) => {
                        if (sortStatus === 'none') return 0;
                        if (a.isPaid === b.isPaid) return 0;
                        if (sortStatus === 'pending') return a.isPaid ? 1 : -1;
                        return a.isPaid ? -1 : 1;
                      }).map((t) => (
